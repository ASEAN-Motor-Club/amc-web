<script lang="ts">
  ...
  const parseTrackData = (text: string) => {
    try {
      ...
      if (!validate.success) {
        showModal({
          title: siteLocale.msg['track_editor.select_track.error.title'](),
          message:
            siteLocale.msg['track_editor.select_track.error.invalid_track']() +
      ...
      showModal({
        title: siteLocale.msg['track_editor.select_track.error.title'](),
        message: siteLocale.msg['track_editor.select_track.error.load_error'](),
      });
    }
  };
  ...
  const handleLoadFromClipboard = async () => {
    ...
    if (!clipboard) {
      showModal({
        title: siteLocale.msg['track_editor.select_track.error.title'](),
        message: siteLocale.msg['track_editor.select_track.error.clipboard_empty'](),
      });
      return;
    }
  ...
  const tryFetchTrack = async (uri: string, signal: AbortSignal) => {
    try {
      ...
      showModal({
        title: siteLocale.msg['track_editor.select_track.error.cannot_load_track'](),
        message: siteLocale.msg['track_editor.select_track.error.cannot_load_track_desc'](),
      });
    }
  };
...
<div class="flex h-full w-full p-8">
  ...
  >
    {siteLocale.msg['track_editor.select_track.loading_track']()}
  </Modal>
  <div
    class={[
      'rounded-4xl before:z-1000 relative flex h-full w-full flex-col items-center justify-center overflow-hidden border-8 border-dotted border-black/10 p-8 before:absolute before:inset-0 before:bg-black/20 motion-safe:transition-all dark:border-white/10 before:dark:bg-white/20',
      {
  ...
  >
    ...
    <Button onClick={handleSelectFileClick}>
      {siteLocale.msg['track_editor.select_track.select_file']()}
    </Button>
    <input
      type="file"
