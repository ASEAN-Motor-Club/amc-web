...
<Card class="flex w-full max-w-120 flex-row items-center gap-2" {loading}>
  ...
  <div class="flex flex-1 flex-col justify-between sm:flex-row sm:gap-2">
    <div>
      ...
      <div class="text-text-700 dark:text-text-300 text-xs">
        {#if loading}
          .
        {:else if time?.championship_point}
          <span class="font-semibold"
            >{m['championship.results.pts']({
    ...
    <div class="flex items-baseline gap-1 sm:ml-auto sm:block sm:items-center sm:justify-between">
      <div class="text-center font-semibold sm:text-right">
        ...
        {:else if time?.finished}
          {formatTime(netTime)}
        {:else}
          <span class="italic opacity-50">{m['championship.event.dnf']()}</span>
        {/if}
      </div>

      <div class="text-text-800 dark:text-text-200 text-center text-xs sm:text-right">
        {#if loading}
          .
        {:else if time?.finished}
          {#if index > 0}
          ...
          {:else}
            {m['championship.results.fastest']()}
          {/if}
        {/if}
      </div>
    </div>
