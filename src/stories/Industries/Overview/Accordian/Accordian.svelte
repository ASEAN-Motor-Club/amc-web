<script lang="ts">
  ...
  let { factoryName, isManufacturer, inventoryStatus, children, index }: AccordianProps = $props();
</script>

<div class="relative h-[46px] w-[420px] !p-0">
  <Card
    class={[
      'accordian absolute p-0 overflow-hidden',
      {
        'absolute border-4 border-t-0 border-red-700':
          inventoryStatus === INV_STATUS.ALERT && isManufacturer,
      },
      {
        'border-4 border-t-0 border-orange-500':
          inventoryStatus === INV_STATUS.ALERT && isManufacturer,
      },
      {
        'border-yellow border-4 border-t-0':
          inventoryStatus === INV_STATUS.CAUTION && isManufacturer,
      },
      { 'border-t-0 border-green-600': inventoryStatus == INV_STATUS.NORMAL && isManufacturer },
      { 'z-10000': ActiveAccordian.atIndex === index },
    ]}
  >
...
> -->
    ...
    <div
      class="panel flex cursor-pointer justify-between"
    ...
    >
      <div class="w-full self-center">
        {#if inventoryStatus > INV_STATUS.CAUTION}
          <svg
            fill="#e7af13"
            class="alert inline animate-pulse pl-2"
            viewBox="0 0 32 32"
            version="1.1"
          ...
          >
        {/if}
        <p class="inline p-4 pl-2 text-left text-xl font-extrabold text-white">
          {factoryName}
        </p>
      </div>
      <!-- Credit: https://www.svgrepo.com/svg/521469/arrow-down -->
      <svg
        class:extend={ActiveAccordian.atIndex === index}
    ...
    {#if ActiveAccordian.atIndex === index}
      <div
        class="accordianBody z-9999 w-full px-2 pt-2"
