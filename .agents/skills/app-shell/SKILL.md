...
Fonts are self-hosted via `@fontsource` packages (Noto Sans variants per script, Kanit, Koulen, Playfair Display, Great Vibes). `src/lib/font-sans-em.ts` holds font metric helpers. The web app manifest is generated by the custom Vite plugin in `vite-plugins/webmanifest.ts`.
