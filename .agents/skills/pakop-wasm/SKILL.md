...
- C# project at `wasm/pakop/` (`Pakop.csproj`, .NET 10 browser WASM, no Blazor); provides pak-file inspection and hashing (`list`, `list_hash`, `get_datatables_names`, `print_exports`) used by the `/pak` routes (`src/routes/pak/`). Built on [CUE4Parse](https://github.com/FabianFG/CUE4Parse) — see [[cue4parse]] knowledge before changing parsing logic.
...
Related: [[architecture]], [[git-hooks]], [[path-aliases]], [[cue4parse]]
